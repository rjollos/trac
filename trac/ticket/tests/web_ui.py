...
class TicketModuleTestCase(unittest.TestCase):
    ...
    def _prepare_ticket_request(self, tid):
        ...
        req = MockRequest(
            ...
            args=dict(id=str(tid), action='leave', submit='Submit changes',
                      comment='', replyto='', start_time=start_time,
                      view_time=start_time, **fields)
        )
        return req

    def _submit_ticket_request(self, req):
        mod = TicketModule(self.env)
        self.assertTrue(mod.match_request(req))
        try:
            data = mod.process_request(req)[1]
    ...
    def test_comment_save(self):
        ...
        self._submit_ticket_request(req)

        req = self._prepare_ticket_request(1)
        req.args['comment'] = 'The comment'
        data = self._submit_ticket_request(req)
